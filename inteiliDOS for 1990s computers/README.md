...
This directory is an independent copy of the HP edition, retargeted for broad
1990s PC compatibility. The original `inteiliDOS for HP` directory is unchanged.

The compatibility baseline is:

- Intel 80386 instruction set (`-march=i386`), with no MMX, SSE, CMOV, or
  unconditional CPUID use
- legacy BIOS boot, Multiboot1, and a raw CHS floppy image
- standard VGA 80x25 text mode
- dual 8259 PIC and 8254 PIT
- PS/2 keyboard as the default input path
- PIO IDE compatibility ports first; PCI-native probing remains a fallback
- GRUB-provided E820 memory map when available, with basic BIOS memory totals
  on the raw floppy path

“Almost every” cannot mean every machine: unusual proprietary laptops, systems
without VGA, 286 and older CPUs, non-PC-compatible firmware, unsupported SCSI
controllers, and machines with less than 4 MB RAM remain outside the target.
Optional HP-specific AC'97 capture and late-1990s PCI features are retained in
the source but are not required for boot, display, timing, or keyboard input.

Build this edition with:
...
Outputs are written under `build_1990s/`.
...
> **This is the broad 1990s compatibility edition.** It uses an Intel 80386
> compiler baseline and legacy PC interfaces. Hardware-specific extras are
> opportunistic and silently remain unavailable when matching hardware is absent.
...
> **Minimum requirements:** PC-compatible Intel 80386 or newer, VGA-compatible
> display, PS/2-compatible keyboard controller, BIOS boot support, and 4 MB RAM.
> **8 MB RAM is recommended for LaunchPad** because programs load at 5 MB.
