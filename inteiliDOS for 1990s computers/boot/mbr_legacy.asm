...
; Universal CHS Boot Sector — 386+ / ISA / El Torito Floppy-Emulation
; =============================================================================
;
...
; --------------------
;   • Intel 80386DX and 80386SX systems with enough RAM
...
_start:
    ...
    ; ── Step 2: Ask the BIOS to enable A20, then use Fast A20 as fallback ────
    ; INT 15h AX=2401h is ignored by old BIOSes that do not implement it.
    mov  ax, 0x2401
    int  0x15
    in   al, 0x92
    test al, 0x02
...
.a20_ok:
    ...
    ; ── Step 3: Gather conventional and extended memory totals ──────────────
    ; Construct the beginning of a Multiboot1 information structure.  INT 12h
    ; works on all PC-compatible BIOSes; AH=88h is the early-1990s fallback
    ; for extended memory.  GRUB boots use their richer E820-derived map.
    int  0x12
    mov  [mb_info + 4], ax
    xor  eax, eax
    mov  ah, 0x88
...
.mem_done:
    ...
    ; ── Step 5: Enter unreal mode — load FS with a 4 GB data descriptor ───────
...
.unreal:
    ...
    ; ── Step 6: Detect drive geometry via INT 13h AH=0x08 ─────────────────────
...
.geo_done:
    ...
    ; ── Step 7: Initialise CHS state and destination pointer ──────────────────
    mov  byte [chs_cyl],  0
    mov  byte [chs_head], 0
...
.pm32:
    ...
    ; Present the basic BIOS totals using the Multiboot1 calling convention.
    mov  eax, 0x2BADB002
    mov  ebx, mb_info
...
align 4
mb_info:
    dd 1                           ; flags: mem_lower and mem_upper are valid
    dd 640                         ; conventional KB; replaced by INT 12h
    dd 3072                        ; extended KB; replaced by INT 15h AH=88h
