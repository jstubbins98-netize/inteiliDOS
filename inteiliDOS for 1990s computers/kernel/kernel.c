...
static void print_banner(size_t mem_kb) {
    ...
    vga_puts("  Machine:\n");
    vga_printf("    Intel 80386-compatible Protected Mode CPU\n");
...
void kernel_main(uint32_t mb_magic, uint32_t mb_info_phys) {
    ...
    *(volatile uint16_t *)0xB8006u = 0x0F34u;   /* '4' bright-white on black */
    ...
    speaker_boot_chime();
    /*
     * PS/2 is the universal input path.  UHCI probing is intentionally
     * disabled in this edition: many early-1990s ISA/EISA systems have no
     * PCI configuration mechanism, and late systems retain PS/2 support.
     */
    vga_set_color(VGA_COLOR_LIGHT_GREEN, VGA_COLOR_BLACK);
    vga_puts("[OK]\n");
    vga_set_color(VGA_COLOR_LIGHT_GREY, VGA_COLOR_BLACK);
