...
The script:
1. Starts Willburd the Setup Wizard
2. Offers an `H` help choice for every hardware question
3. Creates `configurations/config.h`
4. Checks all dependencies
5. Runs `cmake` with the `cmake/i686-elf.cmake` toolchain file
6. Builds the kernel and the selected floppy and/or CD-ROM image

To generate only the hardware header:

```bash
./build.sh --config-only
```

For an unattended safe-default build:
...
```bash
qemu-system-i386 -cdrom build_1990s/inteiliDOS_1990s.iso -m 16 -no-reboot
```
