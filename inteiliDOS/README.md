...
| **InteiliBASIC cassette I/O** | `CSAVE` encodes the BASIC program as Kansas City Standard (KCS) audio tones, writes a RIFF/WAV temp file to `C:\CASSETTE.WAV`, plays the tones live through the PC speaker, then erases the file. `CLOAD` displays "Press PLAY on your tape recorder", listens for an incoming KCS signal for 10 seconds, and decodes it back into memory (audio capture requires the Intel HDA driver). |
...
| Intel HD Audio (HDA) driver for CLOAD audio capture | 🔧 Planned |
