...
static int cmd_help(int argc, const char *argv[]) {
    ...
    println("  TALK     - InteiliTalk text-to-speech (PC speaker)");
...
static int cmd_talk(int argc, const char *argv[]) {
    if (argc >= 1) {
        /* "TALK Hello world" — join all tokens into one sentence and speak */
        char sentence[256];
        int pos = 0;
...
static int cmd_demo(int argc, const char *argv[]) {
    ...
    vga_putchar('\n');
    vga_set_color(VGA_COLOR_DARK_GREY, VGA_COLOR_BLACK);
    println("  Launch with:  TALK");
    println("  Or speak directly:  TALK Hello world");
    vga_set_color(VGA_COLOR_WHITE, VGA_COLOR_BLACK);
    vga_putchar('\n');
