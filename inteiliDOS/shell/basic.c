 ...
 * ================================================================ */
static int cload_audio_backend;

static int configured_audio_has_signal(void) {
#if CONFIG_AUDIO_MODE == CONFIG_AUDIO_AC97
    cload_audio_backend = CONFIG_AUDIO_AC97;
    return ac97_has_signal();
#elif CONFIG_AUDIO_MODE == CONFIG_AUDIO_HDA
    cload_audio_backend = CONFIG_AUDIO_HDA;
    return hda_has_signal();
#elif CONFIG_AUDIO_MODE == CONFIG_AUDIO_AUTO
    if (ac97_has_signal()) {
        cload_audio_backend = CONFIG_AUDIO_AC97;
        return 1;
    }
...
static int configured_audio_capture_byte(void) {
    if (cload_audio_backend == CONFIG_AUDIO_HDA)
        return hda_capture_kcs_byte();
    if (cload_audio_backend == CONFIG_AUDIO_AC97)
        return ac97_capture_kcs_byte();
    return -2;
}
...
static void do_cload(void) {
    ...
    while (elapsed < timeout_ms) {
        ...
        /* Capture through the backend selected in generated config.h. */
        if (configured_audio_has_signal()) {
            ...
            for (;;) {
                b = configured_audio_capture_byte();
