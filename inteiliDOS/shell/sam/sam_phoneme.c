...
void sam_set_input(const char *text) {
    int i;
    /* The reciter (TextToPhonemes) copies all 255 bytes of sam_input[]
     * into its internal buffer regardless of the 0x9B terminator.  If
     * leftover phoneme notation from a previous call sits past position i,
     * the reciter re-processes it as English text and speaks garbage.
     *
     * Fix: pre-fill the entire buffer with '[', the reciter's own stop
     * character.  The first '[' the reciter encounters after the real text
     * causes it to write the 0x9B terminator and return immediately,
     * leaving the rest of the buffer unprocessed.                         */
    for (i = 0; i < 256; i++) sam_input[i] = '[';
    i = 0;
    while (text[i] && i < 254) {
    ...
    /* sam_input[i] is already '[' from the pre-fill above — reciter stop */
}
