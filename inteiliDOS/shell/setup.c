...
static const uint8_t mbr_bootstrap[446] = {
    ...
    0x52, 0x90,                    /* push dx; nop  — save DL on stack before INT 13h.
                                    * Phoenix BIOS (G580/SecureCore) trashes DL *and*
                                    * BH across INT 13h AH=42h.  Stack is the only
                                    * register state that survives; popped at 0x71.  */
    0xCD, 0x13,                    /* int  0x13                         */
...
static const uint8_t vbr_code[512] = {
    ...
    /* 0x036: loop iteration counter — starts at 64 (64 × 8 sectors = 512 = 256 KB).
     * The load loop uses DEC + JNZ, so it counts down 64 → 1 → 0 (exits at 0). */
    0x40,
     ...
     * avoided after the first one.  On the Lenovo IdeaPad G580 (HM76/CSM),
     ...
     * this Lenovo, causing an infinite loop.  Port 0x92 is always safe.     */
     ...
     * normal boot path; the BIOS (Lenovo G580/Phoenix CSM) leaves the display
     * in the splash-screen graphics mode, so 0xB8000 writes are invisible
    ...
    /* 0x0D4 */ 0xB9,0x00,0x00,0x01,0x00,        /* mov  ecx, 0x10000  (65536 dw) */
    /* 0x0D9 */ 0xF3,0xA5,                      /* rep  movsd  (256 KB)          */
