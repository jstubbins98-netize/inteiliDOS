 ...
 *   Escape              Cancel edit / return to navigate
...
static void sh_draw_title(void) {
    ...
    const char *hints = "  Ctrl+Q=Quit  Ctrl+S=Save  ENTER=Edit  DEL=Clear  ";
...
static void sh_draw_hints(void) {
    ...
    const char *h = "  ARROWS=Navigate  ENTER=Edit  DEL=Clear  "
                    "=SUM(A1:G10) / =AVG(A1:A10)  Ctrl+Q=Quit";
 ...
 * Cell editing
 * ========================================================================= */
/* Edit the selected cell in-place.  The formula bar shows the raw content.
 * Returns when the user presses Enter or Escape.
 */
/* Returns 0 normally, 1 if the user typed "quit" to return to IntelliShell. */
static int sh_edit_cell(void) {
    ShCell *cell = &sh_cells[sh_cur_row][sh_cur_col];
    char buf[SH_CMAX + 1];
    /* Copy current content into edit buffer */
    for (int i = 0; i <= SH_CMAX; i++) buf[i] = cell->str[i];
    int len = sh_strlen(buf);

    /* Show edit mode in status */
    sh_draw_status("  EDITING -- Enter=Accept  Esc=Cancel  Type quit=Exit");

    /* Render edit state in formula bar */
    while (1) {
        /* Redraw formula bar with edit buffer */
        sh_fill_row(SH_FBAR_ROW, VGA_COLOR_WHITE, VGA_COLOR_BLACK);
        const char *pfx = "  EDIT: ";
        for (int i = 0; pfx[i]; i++)
        ...
        if (k == KEY_ENTER || k == '\r') {
            buf[len] = '\0';
            /* "quit" typed into a cell → return to IntelliShell */
            if ((buf[0]=='q'||buf[0]=='Q') &&
                (buf[1]=='u'||buf[1]=='U') &&
                (buf[2]=='i'||buf[2]=='I') &&
                (buf[3]=='t'||buf[3]=='T') &&
        ...
        if (k == KEY_ESCAPE) {
            return 0;   /* discard changes */
        }
        if (k == KEY_BACKSPACE && len > 0) {
            buf[--len] = '\0';
        } else if (k >= 0x20 && k < 0x7F && len < SH_CMAX) {
            buf[len++] = (char)k;
            buf[len]   = '\0';
        }
    }
...
void sheets_run(void) {
    ...
    while (1) {
        ...
        if (k == KEY_ENTER || k == '\r') {
            if (sh_edit_cell()) break;   /* "quit" → return to IntelliShell */
            sh_redraw_all("  Ready");
