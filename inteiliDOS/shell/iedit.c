...
void iedit_run(const char *name) {
    ...
    for (;;) {
        ...
        /* ---- Editing ---- */
            ...
            /* Typing "quit" alone on an empty line returns to IntelliShell */
            if (tlen[cur_row] == 4 &&
                (text[cur_row][0]=='q'||text[cur_row][0]=='Q') &&
                (text[cur_row][1]=='u'||text[cur_row][1]=='U') &&
                (text[cur_row][2]=='i'||text[cur_row][2]=='I') &&
                (text[cur_row][3]=='t'||text[cur_row][3]=='T')) {
                /* Erase the word so it isn't left in the buffer */
                tlen[cur_row] = 0;
                text[cur_row][0] = '\0';
                break;
            }
