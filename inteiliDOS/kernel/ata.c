...
static int pata_detect(void) {
    ...
    /* Software-reset both channels so a BIOS-dirtied controller starts clean.
     * Use g_ata_ctrl so we reset whichever ports are actually in use.       */
    for (int ch = 0; ch < 2; ch++) {
        uint16_t ctrl = g_ata_ctrl[ch * 2];    /* slot 0 = ch0, slot 2 = ch1 */
        outb(ctrl, 0x04);                      /* assert SRST               */
        for (int i = 0; i < 16; i++) inb(ctrl);/* hold ≥ 5 µs              */
        outb(ctrl, 0x00);                      /* release SRST              */
        /* Wait for drives to come back (BSY clears, typically < 2 ms) */
        for (uint32_t w = 0; w < 2200000; w++) inb(ctrl);
    }
