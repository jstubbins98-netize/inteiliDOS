...
static int pata_detect(void) {
    ...
    /* Update port addresses from PCI BARs only when selected in config.h. */
...
int ata_detect(ata_drive_t out[ATA_MAX_DRIVES]) {
    ...
    for (int i = 0; i < ATA_MAX_DRIVES; i++) {
        ...
        out[i].drive_index = (uint8_t)i;
        out[i].total_sectors = 0;
        out[i].model[0] = '\0';
    }
    return 0;
#endif
    ...
    /* Probe AHCI (fills slots 4-7) only on configured PCI systems. */
#if CONFIG_ENABLE_AHCI
    total += ahci_detect(4);
#endif
