...
void kernel_main(uint32_t mb_magic, uint32_t mb_info_phys) {
     ...
     *
     * The Lenovo G580 (Phoenix/CSM) hands off to the MBR with the display
     * already in VGA text mode 3: the error-handler path in the VBR uses
     * INT 10h AH=0Eh (teletype) which proves the BIOS text framebuffer is
     * live.  Our VBR never calls INT 10h itself (AH=00h/set-mode hangs on
     * this BIOS), so the display mode is unchanged when we enter PM.
     *
     * Calling vga_set_mode3() here reprograms raw VGA I/O ports.  On Intel
     * HD Graphics (Sandy/Ivy Bridge), the display engine does NOT honour
     * direct VGA register writes after the VBIOS has already configured it;
     * the call corrupts the live text-mode state and produces a blank blue
     * screen.  Skip it: just write to 0xB8000 in the mode the BIOS left.
     *
