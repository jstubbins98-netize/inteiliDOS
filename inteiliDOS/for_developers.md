...
│  (35+ cmds)  │  IEdit │ BASIC │ Sheets │ Talk  │  shell/iedit.c, basic.c
│              │  DEMO  │ TOUR  │ QUIT   │       │  shell/sheets.c, talk.c, tour.c
├──────────────┴──────────────────────────────┤
│               Kernel Services               │
...
- **Intel HD Audio (HDA) driver for CLOAD audio capture** — implement `kernel/hda.c` / `kernel/hda.h`. The CLOAD decoder skeleton in `do_cload()` (`shell/basic.c`) is complete and contains an inline hook comment showing exactly which two functions to implement (`hda_has_signal()` and `hda_capture_kcs_byte()`). PCI class 0x04 / subclass 0x03 locates the HDA controller. See §4.10 for the full driver contract and integration steps.
...
- **InteiliBASIC BEEP statement** — wire `speaker_beep(freq_hz, duration_ms)` into the InteiliBASIC `BEEP` statement, which currently does nothing. The speaker API already exists in `timer.h` and is already used by InteiliTalk (`shell/talk.c`); only the BASIC statement itself needs implementing.
...
- **Persistent file system** — design a simple flat filesystem (FAT12 or a custom layout) on top of the ATA disk I/O driver. Wire it into the existing simulated directory structure in `commands.c`. The ATAPI CD-ROM driver already provides read-only ISO 9660 sector access as a foundation.
...
- **OHCI/EHCI USB keyboard** *(UHCI already implemented; OHCI and EHCI are planned tasks)* — extend `kernel/usb.c` to support OHCI (prog_if 0x10) and EHCI (prog_if 0x20) host controllers, for hardware or hypervisors that do not expose a UHCI controller. See §4.7 for data-structure alignment requirements and §4.8 for the ATAPI extension guide as a structural reference. Also planned: detect and gracefully handle USB device hot-unplug mid-session (currently the polling TD can fault if the device disappears while the Active bit is being checked).
...
- **Legacy build: OHCI/EHCI validation** — the legacy build targets real 1990s hardware. Before adding new drivers, verify them under QEMU with `-cpu pentium` (or `-cpu 486`) to catch any i686-only instructions early.
