...
- **OHCI/EHCI USB keyboard** *(UHCI already implemented)* — extend `kernel/usb.c` to support OHCI (prog_if 0x10) and EHCI (prog_if 0x20) host controllers, for hardware or hypervisors that do not expose a UHCI controller. See §4.8 for the ATAPI extension guide as a structural reference.
